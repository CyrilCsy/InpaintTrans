...
from src.models import BaseLine as baseline
...
class BaseLine():
    def __init__(self, config):
        ...
        if self.config.MODE == 2:
            ...
            self.val_dataset = Dataset(config, config.VAL_FLIST, config.VAL_MASK_FLIST, augment=False, mask_reverse=config.MASK_REVERSE, training=False)
    ...
    def sample(self, it=None):
        ...
        iteration = self.model.iteration
        inputs = images * masks.float() + (1 - masks.float())
        outputs = self.model(images, masks)
        outputs_comp = (images * masks) + (outputs * (1 - masks))
